...
public class ActionHandler {
    ...
    public static final String SYSTEMUI_TASK_MAX_BRIGHTNESS = "task_brightness_mode";
    ...
    static enum SystemAction {
        ...
        MaxBrightness(SYSTEMUI_TASK_MAX_BRIGHTNESS, SYSTEMUI, "label_action_max_brightness", "ic_max_brightness");
...
//            toggleOneHandedMode(context, "right");
            return;
        } else if (action.equals(SYSTEMUI_TASK_MAX_BRIGHTNESS)) {
            maxBrightness(context);
        }
    }
...
/*
    ...
    */
    ...
    public static void maxBrightness(Context context) {
        ...
        try  {
        mCurrentMode = android.provider.Settings.System.getInt(
            context.getContentResolver(),
            android.provider.Settings.System.SCREEN_BRIGHTNESS_MODE);
        } catch (SettingNotFoundException e){
            Log.i(TAG, "Unable to get setting " + e);
        }

        if (mCurrentMode == 1){
            int duration = Toast.LENGTH_SHORT;

            Toast toast = Toast.makeText(context, "Auto mode enabled", duration);
            toast.show();
